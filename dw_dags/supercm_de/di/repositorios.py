...
class ExecutionLog(object):
    TABLE_NAME = '[SuperCM].[vw].[CM_ExecutionLog]'
    ...
    def findAll_minimum_by_Canal_DataExecucaoIsToday_SucessoIsTrue_IdExecutionNotNegativo(canal, **kwargs):
        ...
        _stm = '''
            ...
            WHERE B.%s = 1
                AND CAST(A.DataExecucao AS DATE) = CAST(GETDATE() AS DATE)
                AND A.Sucesso = 1
                AND A.IdExecution > 0
            ORDER BY 1
...
class OutputSegmentadorEmail(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_OutputSegmentador_email]'
    ...
    GCR = 'GCR'
    ID_EXECUTION_REGUA = 'IdExecutionRegua'
    ...
    def find_minimum_by_IdDefinition_IdExecution_DataTransmissaoIsNull_GCIsFalse_TriboolSegmentaIp(id_definition,
        ...
        _stm = '''
            ...
            AND GC = 0
            AND GCR <> 1
        ''' % ViewOutputSegmentadorEmail.TABLE_NAME
        ...
        for _ in database.do_db_query_fetchall(_stm, parameters=_p, **kwargs):
            _entity = {ViewOutputSegmentadorEmail.EMAIL: _[0],
                       ViewOutputSegmentadorEmail.CONTA_CORRENTE_ID: _[1],
                       ViewOutputSegmentadorEmail.HEADER_CAMPANHA: _[2],
                       ViewOutputSegmentadorEmail.VALUES_CAMPANHA: _[3],
                       }
            _ret.append(_entity)
    ...
    @staticmethod
    def insertAll_dadosTransmissao_by_IdDefinition_IdExecution (id_definition,
                                                                id_execution,
                                                                tribool_segmenta_ip,
                                                                **kwargs):
        ...
        try:
            ...
            __msg_error = 'ERRO INESPERADO para o metodo: [%s] Causa[%s] \n\tStackTrace[%s]' % (id_definition,
                                                                                             e,
                                                                                             utils.stack_trace(e))
        ...
        _upsert = '''
                    ...
                    LEFT JOIN %s as j
                        ON i.IdDefinition = j.IdDefinition
                        AND i.IdExecution = j.IdExecution
                    
                    WHERE i.IdDefinition = ?
                        AND i.IdExecution = ?
                        AND i.DataTransmissao IS NULL
                        AND j.IdDefinition IS NULL
                ''' % (OutputSegmentadorEmail.TABLE_NAME, ViewOutputSegmentadorEmail.TABLE_NAME,
                       OutputSegmentadorEmail.TABLE_NAME)

        _is_true_or_false = Tribool(None).count(tribool_segmenta_ip.value) < 1
        if _is_true_or_false:
            _p.append(utils.from_bool_2_int(tribool_segmenta_ip.value))
            _upsert = '%s AND i.IndIPAtivo = ?' % _upsert
...
class OutputSegmentadorSMS(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_OutputSegmentador_SMS]'
    ...
    def findAll_by_IdDefinition_IdExecution_DataTransmissaoIsNull_notGC(id_definition, id_execution, **kwargs):
        ...
        _stm = '''
            ...
            WHERE IdDefinition = ?
                ...
                AND GC = 0
                AND GCR <> 1
        ''' % ViewOutputSegmentadorSMS.TABLE_NAME

        _ret = []
        for _ in database.do_db_query_fetchall(_stm, parameters=_p, **kwargs):
            _entity = {ViewOutputSegmentadorSMS.CONTA_CORRENTE_ID: _[0],
                       ViewOutputSegmentadorSMS.ID_DEFINITION: _[1],
                       ViewOutputSegmentadorSMS.ID_EXECUTION: _[2],
                       ViewOutputSegmentadorSMS.TELEFONE: _[3],
                       ViewOutputSegmentadorSMS.MENSAGEM: _[4],
                       }
            _ret.append(_entity)
    ...
    @staticmethod
    def insertAll_dadosTransmissao_by_IdDefinition_IdExecution(id_definition, id_execution, **kwargs):

        try:
            if not (id_definition and id_execution):
                raise ValueError('Informe idDefinition/idExecution corretamente! [%s,%s]' % (
                    id_definition, id_execution))
        except Exception as e:
            __msg_error = 'ERRO INESPERADO para o metodo: [%s] Causa[%s] \n\tStackTrace[%s]' % (id_definition,
                                                                                                e,
                                                                                                utils.stack_trace(e))
        ...
        _upsert = '''
                        ...
                        WHERE i.IdDefinition = ?
                            ...
                            AND i.DataTransmissao IS NULL
                            AND j.IdDefinition IS NULL
                    ''' % (OutputSegmentadorSMS.TABLE_NAME, ViewOutputSegmentadorSMS.TABLE_NAME,
...
class OutputSegmentadorPush(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_OutputSegmentador_PUSH]'
    ...
    def findAll_by_IdDefinition_IdExecution_DataTransmissaoIsNull_notGC_DevicePlataforma(id_definition,
        ...
        _stm = '''
            ...
            ''' % (_all_columns(ViewOutputSegmentadorPush.ALL_COLUMNS), ViewOutputSegmentadorPush.TABLE_NAME)

        _ret = []
        for _ in database.do_db_query_fetchall(_stm, parameters=_p, **kwargs):
            _entity = ViewOutputSegmentadorPush._load_entity(_)
            _ret.append(_entity)

        return _ret

    @staticmethod
    def insertAll_dadosTransmissaoPush_by_IdDefinition_IdExecution_Device(id_definition,
                                                                          id_execution,
                                                                          device_plataforma,
                                                                          **kwargs):
        ...
        try:
            if not (id_definition and id_execution and device_plataforma):
                raise ValueError('Informe Parametros corretamente! [%s, %s, %s]' % (
                    id_definition, id_execution, device_plataforma))
        except Exception as e:
            __msg_error = 'ERRO INESPERADO para o metodo: [%s] Causa[%s] \n\tStackTrace[%s]' % (id_definition,
                                                                                                e,
                                                                                                utils.stack_trace(e))
        ...
        _upsert = '''
                    ...
                    WHERE i.IdDefinition = ?
                        AND i.IdExecution = ?
                        AND i.DataTransmissao IS NULL
                        AND i.DevicePlataforma = ?
                        AND NOT EXISTS (
                            SELECT j.[ContaCorrenteId], 
                                j.[IdDefinition], 
                                j.[IdExecution], 
                                j.[DeviceId], 
                                j.[DevicePlataforma], 
                                ...
                                j.[IdExecutionRegua] 
                            FROM %s j
                            WHERE j.IdDefinition = ?
                                AND j.IdExecution = ?
                                AND j.DevicePlataforma = ?)
                ''' % (OutputSegmentadorPush.TABLE_NAME, ViewOutputSegmentadorPush.TABLE_NAME,
...
# View SuperCM
class ViewOutputSegmentadorEmail(object):
    TABLE_NAME = '[SuperCM].[vw].[CM_OutputSegmentador_email]'
...
# View SuperCM
class ViewOutputSegmentadorSMS(object):
    TABLE_NAME = '[SuperCM].[vw].[CM_OutputSegmentador_SMS]'
...
# View SuperCM
class ViewOutputSegmentadorPush(object):
    TABLE_NAME = '[SuperCM].[vw].[CM_OutputSegmentador_PUSH]'
    ...
    @staticmethod
    def _load_entity(entidade):
        _entity = {ViewOutputSegmentadorPush.CONTA_CORRENTE_ID: entidade[0],
                   ViewOutputSegmentadorPush.ID_DEFINITION: entidade[1],
                   ViewOutputSegmentadorPush.ID_EXECUTION: entidade[2],
                   ViewOutputSegmentadorPush.DEVICE_ID: entidade[3],
                   ViewOutputSegmentadorPush.DEVICE_PLATAFORMA: entidade[4],
                   ViewOutputSegmentadorPush.ASSUNTO: entidade[5],
                   ViewOutputSegmentadorPush.MENSAGEM: entidade[6],
                   ViewOutputSegmentadorPush.GC: entidade[7],
                   ViewOutputSegmentadorPush.DATA_TRANSMISSAO: entidade[8],
                   ViewOutputSegmentadorPush.GCR: entidade[9]
                   }

        all_columns = ViewOutputSegmentadorPush.ALL_COLUMNS
        if len(all_columns) != len(_entity):
            raise Exception('[OutputSegmentadorPush] _load_entity DEVE conter todos colunas de ALL_COLUMNS! '
                            '\nentity[%s] \nall_columns[%s]' % (
                                _entity, all_columns))
...
class Campanha(object):
    TABLE_NAME = '[SuperCM].[vw].[CM_Campanhas]'
...
class CampanhaContatoEmail(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_CampanhaContato_Email]'
...
class CampanhaContatoSMS(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_CampanhaContato_SMS]'
...
class CampanhaContatoPush(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_CampanhaContato_Push]'
...
class CampanhaRetornoEmail(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_CampanhaRetorno_Email]'
...
class ParametroOperacional(object):
    TABLE_NAME = '[SuperCM].[dbo].[CM_ParametroOperacional]'
