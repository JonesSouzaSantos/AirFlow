...
from supercm_de.di.repositorios import ExecutionLog, Canal, ParametroOperacional, Campanha, ViewOutputSegmentadorPush, \
    DispositivoPlataforma, CampanhaContatoPush, OutputSegmentadorPush
    ...
    with closing(database.db_connect(**context)) as conn:
        ...
        for plataforma in DispositivoPlataforma:
            ...
            try:
                ...
                bo.inserir_dadosEnvio(id_definition, id_execution, plataforma)

                logging.info('[Plataforma] [%s] ' % plataforma)
...
class BusinessObject(object):
    ...
    def disparar_push(self, campanha, id_execution, dispositivo_plataforma):
        ...
        for push in output_push:
            ...
            _dipositivo = DispositivoPlataforma.by_value(push[ViewOutputSegmentadorPush.DEVICE_PLATAFORMA]).value.lower()

            b.add_binds({BatchPushBuilder.BIND_PUSH_ID: push[ViewOutputSegmentadorPush.CONTA_CORRENTE_ID],
                         ...
                         BatchPushBuilder.BIND_TITULO: push[ViewOutputSegmentadorPush.ASSUNTO] or 'Superdigital',
                         BatchPushBuilder.BIND_MENSAGEM: push[ViewOutputSegmentadorPush.MENSAGEM]})
    ...
    def obter_output_push(self, id_definition, id_execution, device_plataforma):
        return ViewOutputSegmentadorPush. \
            findAll_by_IdDefinition_IdExecution_DataTransmissaoIsNull_notGC_DevicePlataforma(id_definition,
                                                                                             id_execution,
                                                                                             device_plataforma,
                                                                                             conn=self.conn)

    def inserir_dadosEnvio(self, id_definition, id_execution, device_plataforma):

        logging.info('[inserir_dadosEnvio] '
                     'id_definition [%s] id_execution [%s] device_plataforma[%s]' % (id_definition, id_execution,
                                                                                     device_plataforma))

        OutputSegmentadorPush. \
            insertAll_dadosTransmissaoPush_by_IdDefinition_IdExecution_Device(id_definition,
                                                                              id_execution,
                                                                              device_plataforma,
                                                                              conn=self.conn)
