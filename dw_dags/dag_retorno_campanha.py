...
import superdigital_mdc.di.retorno_campanha as bo
...
_dias_a_considerar = bo.get_dias_a_considerar(**op_kwargs)
dt_ref = date.today() - timedelta(days=int(_dias_a_considerar))
dt_fim = date.today()

# PARA TESTES
# dt_ref = date.today() - timedelta(days=2)
# dt_fim = date(2019, 1, 1)

logging.info('Definindo DAG a partir de [%s] por mais [%s] dias' % (dt_ref, _dias_a_considerar))

dag = DAG('cm_yc__retorno_campanha',
          description='DAG da integração com ALLIN - Analise do Retorno de Campanha.',
...
_op_prefix = 'retorno_campanhas__'
...
with dag:
    ...
    while dt_ref < dt_fim:
        ...
        op = PythonOperator(task_id=_op_name,
                            ...
                            op_args=[dt_ref],
                            op_kwargs=op_kwargs,
                            python_callable=di.analisa_retorno_campanhas,
                            dag=dag)
    ...
    op_resumo = PythonOperator(task_id='%sresumo' % _op_prefix,
                               ...
                               op_args=[op_names],
                               op_kwargs=op_kwargs,
                               python_callable=di.resumo_retorno_campanhas,
                               dag=dag)
    _op_bef >> op_resumo
