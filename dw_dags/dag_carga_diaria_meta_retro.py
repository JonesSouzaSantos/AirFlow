...
def get_main_dag():
    ...
    _id = 'carga_diaria_meta_retro'
    MAIN_DAG_NAME = 'z_eng__%s' % _id

    dataReprocessamentoCarga = Variable.get(Constantes.KEY__DATA_REPROCESSAMENTO_CARGA)

    op_kwargs = {Constantes.KEY__CONNECTION_STRING: Constantes.DEF__CONNECTION_STRING,
                 Constantes.KEY__DATA_REPROCESSAMENTO_CARGA: dataReprocessamentoCarga}
    ...
    task_atual = PythonOperator(task_id='carga_diaria_metadados__consulta_status_logs',
                                ...
                                op_kwargs=op_kwargs,
                                python_callable=cd.atualizar_logs_status_carga_diaria_especifica,
                                dag=main_dag)
