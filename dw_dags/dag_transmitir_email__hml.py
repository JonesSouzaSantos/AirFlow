...
import supercm_de.di.transmite_email as facade
import superdigital_mdc.di.allin as facade_allin
from supercm_de.di.repositorios import ExecutionLog
...
dag = DAG('_hml_alpha0__cm_n5__transmitir_email',
          description='DAG da integração com ALLIN - Atualização das listas de envio de email - Decision Engine (N5).',
...
for executionLog in facade.obter_idDefinition_idExecution_4_transmissao(**op_kwargs):
    ...
    op = PythonOperator(task_id=_op_name,
                        ...
                        op_kwargs=op_kwargs,
                        python_callable=facade.transmitir_email_by_idDefinitionIdExecution,
                        dag=dag)
    _op_bef >> op
    _op_bef = op
...
op_resumo = PythonOperator(task_id='%sresumo' % _op_prefix,
                           ...
                           op_args=[op_names],
                           op_kwargs=op_kwargs,
                           python_callable=facade.resumo_transmissao_lista,
                           dag=dag)
_op_bef >> op_resumo

op_status_upload = PythonOperator(task_id='%sstatus_upload' % _op_prefix,
                           provide_context=True,
                           op_args=[op_names],
                           op_kwargs=op_kwargs,
                           python_callable=facade.check_status_upload,
                           dag=dag)
op_resumo >> op_status_upload
